...
module.exports = async () => {
  ...
  // Unconditionally favor "@serverless/components" if Tencent vendor set by env
  if (process.env.SERVERLESS_PLATFORM_VENDOR === 'tencent') return '@serverless/components';
  ...
  if ((process.argv.length === 2 || process.argv[2] === 'deploy') && isChinaUser) {
    return '@serverless/components';
  }
